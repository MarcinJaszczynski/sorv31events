...
<tr>
    ...
    <td>
        <div ><span class="font-weight-light">razem: </span><span class="font-weight-bold">{{ $payment->plannedPrice * $payment->plannedQty }} 
            @if($payment->planned_currency_id!='')
            @foreach($currencies->where('id', $payment->planned_currency_id) as $currency)
            {{ $currency->symbol }}
            @endforeach
            @else
             PLN
            ...
            @if($payment->planned_exchange_rate != null && $payment->planned_exchange_rate != 1)
             <div class="text-muted">(kurs: {{$payment->planned_exchange_rate}}, razem: {{floatval($payment->plannedPrice) * floatval($payment->plannedQty) * floatval($payment->planned_exchange_rate) }} PLN)</div>
            @endif
        </span></div>
        <div class="font-weight-light"><small class="text-muted">({{ $payment->plannedPrice }} 
            @if($payment->planned_currency_id!='')
            @foreach($currencies->where('id', $payment->planned_currency_id) as $currency)
            {{ $currency->symbol }}
            @endforeach
            @else
             PLN
            @endif x {{ $payment->plannedQty }} szt.)</small></div>
    </td>
    ...
    <td>
        <div ><span class="font-weight-light">razem: </span><span class="font-weight-bold">{{ $payment->price * $payment->qty }} 
            @if($payment->currency_id!='')
            @foreach($currencies->where('id', $payment->currency_id) as $currency)
            {{ $currency->symbol }}
            @endforeach
            @else
             PLN
        ...
        @if($payment->exchange_rate != null && $payment->exchange_rate != 1)
             <div class="text-muted">(kurs: {{$payment->exchange_rate}}, razem: {{floatval($payment->price) * floatval($payment->qty) * floatval($payment->exchange_rate) }} PLN)</div>
            @endif</div>
        <div class="font-weight-light"><small class="text-muted">({{ $payment->price }} 
            @if($payment->currency_id!='')
            @foreach($currencies->where('id', $payment->currency_id) as $currency)
            {{ $currency->symbol }}
            @endforeach
            @else
             PLN
            @endif x {{ $payment->qty }} szt.)</small></div>
    </td>
</tr>
