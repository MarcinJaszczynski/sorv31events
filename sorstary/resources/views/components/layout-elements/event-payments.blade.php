                    ...
                    <th class="table-active p-3">Pilot</th>
                        <td class="p-3">
                            @foreach($currencies as $currency)
                                ...
                                $paymentTemp=0.0; 
                                @endphp
                                @foreach($payments->where('planned_currency_id', $currency->id)->where('payer','pilot') as $payment)
                                @php 
                                $pilotPlannedPLN += $payment->plannedPrice * $payment->plannedQty * $payment->planned_exchange_rate;
                            ...
                            @endforeach
                        <hr>
                        <div>razem: {{$pilotPlannedPLN}}  PLN</div>
                                                
                            (Zaliczka: {!! $event->eventAdvancePayment !!} PLN)
                        </td>
                        <td class="p-3">
                            @foreach($currencies as $currency)
                                @php $paymentTemp=0.0; @endphp
                                @foreach($payments->where('currency_id', $currency->id)->where('payer','pilot') as $payment)
                                @php 
                                $pilotTotalPLN += $payment->price * $payment->qty * $payment->exchange_rate;
                                $paymentTemp += $payment->price * $payment->qty;
                                @endphp
                                @endforeach
                                @if($paymentTemp != 0)
                                <div>{{ $paymentTemp }} {{$currency->symbol}}</div>
                            ...
                            @endforeach
                        <hr>
                        <div>razem: {{$pilotTotalPLN}}  PLN</div>
                    </td>
                    </tr>
                    <tr>
                        ...
                        <td class="p-3">
                             ...
                             @foreach($currencies as $currency)
                                ...
                                $paymentTemp=0.0; 
                                @endphp
                                @foreach($payments->where('planned_currency_id', $currency->id)->where('payer','biuro') as $payment)
                                @php 
                                $officePlannedPLN += $payment->plannedPrice * $payment->plannedQty * $payment->planned_exchange_rate;
                                $paymentTemp += $payment->plannedPrice * $payment->plannedQty;
                                @endphp
                                @endforeach
                                @if($paymentTemp != 0)
                            ...
                            @endforeach
                        <hr>
                        <div>razem: {{$officePlannedPLN}}  PLN</div>
                        ...
                        <td class="p-3">
                            @foreach($currencies as $currency)
                                ...
                                $paymentTemp=0.0; 
                                @endphp
                                @foreach($payments->where('currency_id', $currency->id)->where('payer','biuro') as $payment)
                                @php 
                                $officeTotalPLN += $payment->price * $payment->qty * $payment->exchange_rate;
                                $paymentTemp += $payment->price * $payment->qty;
                                @endphp
                                @endforeach
                                @if($paymentTemp != 0)
                            ...
                            @endforeach
                        <hr>
                        <div>razem: {{$officeTotalPLN}}  PLN</div>
                    </td>
                    </tr>                        
                    <tr class="table-active">
                        <th class="p-3">Suma:</th>
                        <th class="p-3">{{ $pilotPlannedPLN + $officePlannedPLN }} PLN</th>
                        <th class="p-3">{{ $pilotTotalPLN + $officeTotalPLN }} PLN</th>
                    </tr>                        
                </table>
...
<script>
    ...
    function (data) {
        ...
        let plannedCurrencyId = document.querySelector('#eventPaymentEditPlanned_currency_id');
            if(data.payment.planned_currency_id!=null){
            plannedCurrencyId.value = data.payment.planned_currency_id;
        }
        else{
            plannedCurrencyId.value = 1;
        }
        // plannedCurrencyId.value = data.payment.planned_currency_id;
        let plannedExchangeRate = document.querySelector('#edit_plannedExchangeRate');
        if(data.payment.planned_exchange_rate!=null){
            plannedExchangeRate.value=data.payment.planned_exchange_rate
        }
        else
        {
            plannedExchangeRate.value=1;
        }
        ...
        if(data.payment.currency_id!=null){
            currency_id.value = data.payment.currency_id;
        }
        else{
            currency_id.value = 1;
        }
        let exchangeRate = document.querySelector('#edit_exchangeRate');
        if(data.payment.exchange_rate != null){
            exchangeRate.value=data.payment.exchange_rate
        }
        else
        {
            exchangeRate.value=1;
        }
        let acceptedPayment = document.querySelector('#payment_accepted');
        if(data.payment.accepted == 1){
            acceptedPayment.checked=true;
        }
        else {
            acceptedPayment.checked=false;
        }
