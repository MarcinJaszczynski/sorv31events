    ...
    <div class="modal fade" id="contractModal" role="dialog" aria-labelledby="contractLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                {{ Form::label('hotelStr', 'Obiekt noclegowy1:', array('class' => 'awesome')) }}<br />
                @php
                $hotelCount=0;
                $hotelStr='';
                @endphp
                    ...
                    @foreach($event->eventContractor as $contractor)
                        ...
                        @php
                        $hotelCount++;
                        $hotelStr = $hotelStr + $contractor->name;
                        @endphp
                        {!!$contractor->contractor->name!!}
                        {{-- <br /> {!!$contractor->contractor->street!!}, {!!$contractor->contractor->city!!},<br />
                        tel.: {!!$contractor->contractor->phone!!}, email: {!!$contractor->contractor->email!!}<br /> --}}
                    @endif
                    @endforeach
                    @if($hotelCount==0)
                        @foreach($event->hotels->sortBy('eventHotelStartDate') as $hotel)
                        @php                
                        $hotelStr = $hotelStr + '<br />' + $hotel->hotelName +', ' + $hotel->hotelStreet + ', ' + $hotel->hotelCity + ', <br />';
