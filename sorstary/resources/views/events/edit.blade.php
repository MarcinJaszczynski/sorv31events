...
$eventElements = \App\Models\EventElement::orderBy('eventElementStart', 'desc')->where('eventIdinEventElements',
$event->id)->get();
...
$myTodos = \App\Models\Todo::orderBy('last_update', 'desc')->where('event_id', $event->id)->where('executor_id',
Auth::user()->id)->where('status_id', '1')->get();
$pendingTodos = \App\Models\Todo::orderBy('last_update', 'desc')->where('event_id', $event->id)->where('executor_id',
Auth::user()->id)->where('status_id', '4')->get();
$finishedTodos = \App\Models\Todo::orderBy('last_update', 'desc')->where('event_id', $event->id)->where('executor_id',
Auth::user()->id)->where('status_id', '2')->get();
$myOrders = \App\Models\Todo::orderBy('last_update', 'desc')->where('event_id', $event->id)->where('principal_id',
Auth::user()->id)->where('status_id', '1')->get();
$pendingOrders = \App\Models\Todo::orderBy('last_update', 'desc')->where('event_id', $event->id)->where('principal_id',
Auth::user()->id)->where('status_id', '4')->get();
$finishedOrders = \App\Models\Todo::orderBy('last_update', 'desc')->where('event_id', $event->id)->where('principal_id',
Auth::user()->id)->where('status_id', '2')->get();
$newAllTodos = \App\Models\Todo::orderBy('last_update', 'desc')->where('event_id', $event->id)->where('principal_id',
'!=',Auth::user()->id)->where('executor_id', '!=',Auth::user()->id)->where('status_id', '1')->get();
$pendingAllTodos = \App\Models\Todo::orderBy('last_update', 'desc')->where('event_id',
$event->id)->where('principal_id', '!=',Auth::user()->id)->where('executor_id',
'!=',Auth::user()->id)->where('status_id', '4')->get();
$finishedAllTodos = \App\Models\Todo::orderBy('last_update', 'desc')->where('event_id',
$event->id)->where('principal_id', '!=',Auth::user()->id)->where('executor_id',
'!=',Auth::user()->id)->where('status_id', '2')->get();
$acceptedTodos = \App\Models\Todo::orderBy('last_update', 'desc')->where('event_id', $event->id)->where('status_id',
'5')->get();
        ...
        <div class="col-sm-8">
            <div class="row ">
                <div class="col d-flex justify-content-end align-items-center">
                    ...
                    <div>
                        <form method="GET" target=”_blank” action="{{url('reports/accountantpdf')}}"
                            accept-charset="UTF-8">
                            <input type="hidden" name="eventId" value="{{$event->id}}">
                            <button type="submit" class="btn btn-link text-uppercase text-secondary fw-bold">
                                rozliczenie
                            </button>
                        </form>
                    </div>
                    <div>
                        <form method="GET" target=”_blank” action="{{url('/reports/briefcasePdf')}}"
                            accept-charset="UTF-8">
                            <input type="hidden" name="eventId" value="{{$event->id}}">
                    ...
                    <div>
                        <form method="GET" target=”_blank” action="{{url('/reports/programpdf')}}"
                            accept-charset="UTF-8">
                            <input type="hidden" name="eventId" value="{{$event->id}}">
                            <button type="submit" class="btn btn-link text-uppercase text-secondary fw-bold"> program
                            </button>
                        </form>
                    </div>

                    <div>
                        <button type="button" id="contractButton"
                            class="btn btn-link text-uppercase text-secondary fw-bold" data-toggle="modal"
                            data-target="#contractModal">umowa</button>
            ...
            <ul class="nav nav-tabs" id="eventEdit" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="basic-tab" data-bs-toggle="tab" data-bs-target="#basic" type="button"
                        role="tab" aria-controls="basicedit" aria-selected="true">Dane podstawowe</button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="eventelement-tab" data-bs-toggle="tab" data-bs-target="#eventelement"
                        type="button" role="tab" aria-controls="eventelement" aria-selected="false">Program</button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="task-tab" data-bs-toggle="tab" data-bs-target="#task" type="button"
                        role="tab" aria-controls="contact" aria-selected="false">Zadania</button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="payments-tab" data-bs-toggle="tab" data-bs-target="#payments"
                        type="button" role="tab" aria-controls="payments" aria-selected="false">Wydatki</button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="documents-tab" data-bs-toggle="tab" data-bs-target="#documents"
                        type="button" role="tab" aria-controls="documents" aria-selected="false">Dokumenty</button>
                </li>
            </ul>
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade show active" id="basic" role="tabpanel" aria-labelledby="basic-tab">
                    ...
                    <div class="tab-pane fade" id="eventelement" role="tabpanel" aria-labelledby="eventelement-tab">
                        ...
                        <x-layout-elements.event-elements-list :currencies='$currencies' :event='$event'
                            :eventElements='$eventElements' :eventContractors='$eventContractors'
                            :payments='$payments' />
                    </div>
                    <div class="tab-pane fade" id="task" role="tabpanel" aria-labelledby="task-tab">

                        <x-layout-elements.todo-collapse-list :myTodos='$myTodos' :pendingTodos='$pendingTodos'
                            :finishedTodos='$finishedTodos' :myOrders='$myOrders' :pendingOrders='$pendingOrders'
                            :finishedOrders='$finishedOrders' :newAllTodos='$newAllTodos'
                            :pendingAllTodos='$pendingAllTodos' :finishedAllTodos='$finishedAllTodos'
                            :acceptedTodos='$acceptedTodos' :todos='$todos' />
                    </div>
                    <div class="tab-pane fade" id="payments" role="tabpanel" aria-labelledby="payments-tab">

                        <x-layout-elements.event-payments :payments='$payments' :event='$event'
                            :currencies='$currencies' :contractorstypes='$contractorstypes'
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="card-footer clearfix">
                            {{-- <button type="button" class="btn btn-primary " data-toggle="modal"
                                data-target="#todoCreateModal"> Dodaj zadanie</button> --}}
                        </div>
                    </div>
                </div>
            </div>
...
//             console.log(tabLink.getAttribute('data-bs-target'));
//         })
//     })
