...
<div class="wrapper">
  ...
  <nav class="main-header navbar navbar-expand navbar-white navbar-light">
          ...
          @can('user-perm')
                ...
                @can('user-perm')
                  ...
                  <div class="dropdown-menu dropdown-menu-left">
                        ...
                        @can('autor-perm')<a class="dropdown-item" href="{{ route('contractors.create') }}">Nowy kontrahent</a>
                        <a class="dropdown-item" href="{{ route('contractorstypes.index') }}">Typy kontrahentów</a>
                        <a class="dropdown-item" href="{{ route('contractorstypes.create') }}">Nowy typ kontrahenta</a>
                        @endcan
                    </div>
                     <a id="navbarDropdown" class="nav-link dropdown-toggle" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                      Kontrahenci
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item">
        <a class="nav-link" href="/todo?executor={{ Auth::user()->id }}&urgent=1" alt="Pilne zadania">
          <i class="far fa-bell"></i>
          <span class="badge bg-danger navbar-badge">
            @php
              echo DB::table('todos')->where('executor_id', Auth::user()->id)->where('status_id', '1')->where('urgent','1')->count();  
            @endphp
          </span>
        </a>        
      </li>
      ...
      <li class="nav-item">
        <a class="nav-link" href="/todo?executor={{ Auth::user()->id }}&status=1" class="small-box-footer" alt="Nowe zadania">
      ...
      <li class="nav-item">
        <a class="nav-link" href="events?eventStatus=Potwierdzona" class="small-box-footer" alt="Nowe zadania">
          <i class="far fa-comments"></i>
          <span class="badge bg-success navbar-badge">
            @php
              echo DB::table('events')->where('eventStatus', 'Potwierdzona')->count();  
            @endphp
          </span>
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="events?eventStatus=Anulowana" class="small-box-footer" alt="Nowe zadania">
          <i class="far fa-comments"></i>
      ...
      function lifetimeline(){
          ...
          latestActivity  = ''
            ...
            else 
            {
              for(let i = 0; i < (Object.keys(res.latestActivities).length); i++){
                let created = moment.utc(res.latestActivities[i].created_at,"YYYY-MM-DD\THH:mm:ss\Z").format("YYYY-MM-DD HH:mm")
                let title =''
                let kind =''
                let author = ''
                let link = ''
        ...
        latestActivity += `
          ...
          <span class="text-muted small float-right"><i class="far fa-clock"> </i> `+created+`</span><br />
              ...
              <div>`+title+` <a href=`+link+`><i class="fas fa-edit float-right"></i></a></div>
            </div>
