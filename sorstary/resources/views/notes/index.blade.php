...
@section('content')
<div class="container">
<h1> Komentarze - index </h1>
<form action="{{ route('notes.create')}}" method="get">
                  @csrf
                  <button class="btn btn-success" type="submit">Nowa notatka</button>
            </form>
...
@foreach($data as $note)
    <tr>
      <th scope="row">{{$note->name}}<br> autor: {!! $note->author_id !!}</th>
        ...
        <td> 
          <div class="btn-group" role="group" aria-label="Basic navi">
            ...
            <form action="{{ URL::to('notes/'.$note->id.'/edit') }}" method="get">
                  @csrf
