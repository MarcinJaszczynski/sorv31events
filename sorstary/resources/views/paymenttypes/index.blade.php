...
<h1> TodoStatus - index </h1>
            ...
            <form action="{{ route('paymenttypes.create')}}" method="get">
                  @csrf
      ...
      <th scope="row">{{$type->name}} </th>
        ...
        <td> 
          <div class="btn-group" role="group" aria-label="Basic navi">
            ...
            <form action="{{ URL::to('paymenttypes/'.$type->id.'/edit') }}" method="get">
                  @csrf
                  <button class="btn btn-primary" type="submit"><i class="bi bi-pencil-square"></i>
</button>
            </form>
            <form action="{{ route('paymenttypes.destroy', $type->id)}}" method="post">
                  @csrf
                  @method('DELETE')
